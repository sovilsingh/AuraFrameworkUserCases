<aura:application extends="force:slds">
	<!--<c:ParentEventExample1/>-->
    <!--<c:Parent_A_ApplicationEventExample2/>-->
    <!--<c:NewContactExample3/>-->
    <!--<c:ShowNewContactExample3/>-->
    <!--<c:SystemandBrowserExample5/>-->
    <!--<c:ParentComponentInheritanceExample6/>-->
    <!--<c:ChildComponentInheritanceExample6/>-->
    <!--<c:ChildComponentInheritanceExample7/>-->
    <!--<c:ParentNestedCompositionExample8/>-->
    <!--<c:ParentLightningComponentLifeCycleExample9/>-->
    <!--<c:ParentUnBoundedExpressionExample10/>-->
    <!--<c:ParentBoundedExpressionExample10/>-->
    <!--<c:DyanamicComponentExample11/>-->
    <!--<c:ValidationComponentExample13/>-->
    <!--<c:HandleExceptionsComponentExample14/>-->
    <!--<c:IterationCollectionComponentExample15/>-->
    <!--<c:IterationCollectionSobjectComponentExample15/>-->
    <!--<c:AuraMethodtComponentExample16/>-->
    <!-- <c:AuraActionParentComponentExample17/>-->
    <!--<c:AccountContactComponentExample18 recordId="0012w000017dwMFAAY"/>-->
    <!--<c:DisplayAccountWrapperComponentExample20/>-->
    <c:customLookupComponentExample21/>
</aura:application>